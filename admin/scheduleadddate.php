    ...
    <div id="container">
        ...
        <div id="navigation">
            <nav class="navbar navbar-expand-md navbar-dark bg-dark">
                <span class="navbar-text">Add Class Date</span>
        ...
        <div id="text">
            <section>
                <h4 id="h3" class="title">New Date</h1>
                
            <?php
             $class_id = $_GET['adddate'];

             $readclass = "SELECT * FROM  inspired_yoga_classes  WHERE class_id='$class_id'";

                    $result = $conn->query($readclass);
                    ...
                    while($row = $result->fetch_assoc()){
                        ...
                        $get_title=$row['class_name'];
                        $get_id = $row['class_id'];
                        

                        echo "<form method='POST' action='scheduleadddateupdate.php'>
                                <input type='hidden' value='$get_id' name='changeid'>
                                <p> 
                                   <h4 id='h3'> $get_title</h4>
                                </p>
                                <p> 
                                    <b id='h3'>Date: </b>
                                </p>
                                <p>
                                    <input name='adddate' type='date' data-date='' data-date-format='DD MMMM YYYY' value='2020-04-25' class='form-control input-lg'> 
                                </p>
                                <p> 
                                   <b id='h3'> Time: </b>
                                </p>
                                <p>
                                    <input type='time' name='addtime' class='form-control input-lg'> 
                                </p>
                               
                                <div style='text-align: right; padding-top:2%;'>
                                    <p> 
                                    <a href='scheduleadddateupdate.php' class='btn btn-outline-dark'>back</a>  
                ...
                if(isset($_POST['upload'])){
                    ...
                    $class_time = $_POST['addtime'];
                    $class_day = $_POST['adddate'];
                                        
                   
                    $user = $_SESSION['admindash'];

    			    $insertclass = "INSERT INTO inspired_yoga_class_calendar
                    INNER JOIN inspired_yoga_classes 
                    ON inspired_yoga_class_calendar.class_id = inspired_yoga_classes.class_id 
                    (class_id, class_name, class_time, class_day) 
                    VALUES ('$get_id', '$get_title', '$get_time', '$class_day')";

                    $result = $conn->query($insertclass);

	    		    if(!$result){
		    		    echo $conn->error;
                    }else{
    				    echo "<p> Date for $class_name uploaded. </p>";
	    		    }
		        }
            ?>
